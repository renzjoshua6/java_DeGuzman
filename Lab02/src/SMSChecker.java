...
public class SMSChecker {
    ...
    public static int smsChecker(Map<String, String> smsToCheck, Integer step) {
        ...
        SMS newSMS = new SMS(smsToCheck.get("sender"), smsToCheck.get("message"), smsToCheck.get("recipient"), timeSent, transID);

        String selectQueryPromo = "SELECT promo_code FROM promo WHERE shortCode='" + smsToCheck.get("recipient") + "';";
        ArrayList<String> promoList = DatabaseConnect.retrievePromoList(selectQueryPromo);

        if (promoList.size() == 1) {
            String selectQueryPromoStartDate = "SELECT startDate FROM promo WHERE shortCode='" + smsToCheck.get("recipient") + "';";
            String selectQueryPromoEndDate = "SELECT endDate FROM promo WHERE shortCode='" + smsToCheck.get("recipient") + "';";

            ArrayList<String> startDateList = DatabaseConnect.retrievePromoList(selectQueryPromoStartDate);
            ArrayList<String> endDateList = DatabaseConnect.retrievePromoList(selectQueryPromoEndDate);
    ...
    protected static void systemSMS(String sender, String recipient, Integer code) {
        ...
        SMS sysSMS = new SMS(sender, static_SMS.get(code), recipient, timeSent, transID);
